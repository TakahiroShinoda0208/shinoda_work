...
do
	...
	do	    
		...
		for e in `seq 80 1 99`
		do

		    for f in `seq 60 1 61`

		    do
			
		#ここでnumファイルを計算しちゃう
			if [ $d = 1 ]; then
			    $Norm -f ${FILE}_tmp.tsv -C $Con -day $c -in $f -out $e > ${FILE}_${b}_${e}_${f}.tsv
			else
			    $Norm2 -f ${FILE}_tmp.tsv -C $Con -day $c -in $f -out $e > ${FILE}_${b}_${e}_${f}.tsv
			fi
			
		#売買日判断
			$Judge -f0 $FILE.tsv -f1 ${FILE}_${b}_${e}_${f}.tsv -cut $CUT  > ${FILE}_${b}_${e}_${f}.result.tsv
			
		#statsをまとめた表を出力
			#awk -f $sep ${FILE}.result.tsv > ${FILE}.stats.tsv
			awk -f $sep ${FILE}_${b}_${e}_${f}.result.tsv > ${FILE}.stats.tsv
